...
#pkgs="openssh "
pkgs="bash-completion ffmpeg nano wget python ncdu htop p7zip command-not-found openssl "
apt update -y
apt install -y $pkgs
unset pkgs
...
string="function pip3() { local args=(\"\$@\");[ \"\$1\" == wheel ]&&args+=(--wheel-dir \"\$SD/deb/wheels\");LDFLAGS='-lm -lcompiler_rt' LD='ld' \`which pip3 2>/dev/null\` \"\${args[@]}\"; }"
if ! [ -f "$bashrc" ] || ! grep -q "^$string$" "$HOME/.bashrc"; then
   echo -e "$string" | tee -a "$HOME/.bashrc" >/dev/null
fi
